...
UserSchema.methods = {
  ...
  getPlanner: function(cb){
    this.populate('requirement exemptions selected', function(err, doc){
      ...
      Course.populate(doc, 'requirement.preassigned', function(err, data){
        ...
        // Retrieve all those with double counts
        var doubleCounts = _.filter(combined, function(c, i){        
          return c.doubleCount.length > 0;
        });
        ...
        requirements.forEach(function(r, i){
          ...
          var getDoubleCountNum = function(type){
            var count = {
              minus: 0,
              add: 0
            };

            doubleCounts.forEach(function(c, i){              
              c.doubleCount.forEach(function(d, i){
                if (type === d.replace) {
                  count.minus++;
                }
          ...
          // Get all those that can be replaced
          var doubleCountable = getDoubleCountNum(type);
          var replaceCount = doubleCountable.minus;
          var addCount = doubleCountable.add;
          ...
          var count = r.qtyRequired - arr.length - replaceCount + addCount;
