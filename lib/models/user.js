  ...
  getPlanner: function(cb){
    this.populate('requirement exemptions selected.courseId doubleCount', function(err, doc){
      ...
      Course.populate(doc, 'requirement.preassigned', function(err, data){
        ...
        // TODO: Need to process selected with which category it is suppose to fufil
        ...
        Object.keys(flat).forEach(function(type, i){
          ...
          // Keep track of the number of slots with double counting
          var qtyRequired = category.qty;
          var categoryDoubleCount = doubleCounts[type];

          if (categoryDoubleCount){            
            categoryDoubleCount.forEach(function(dc, i){
              var empty = countEmpty(flat[type].courses);
              if(empty > 0){
                ...
                // These two are not equal it means that one of the selected already filled
                // the slot already
                if(qtyRequired !== currentCount) {
