  ...
  getPlanner: function(cb){
    this.populate('requirement exemptions selected.courseId doubleCount', function(err, doc){
      ...
      Course.populate(doc, 'requirement.preassigned.courseId', function(err, data){
        ...
        allRequirements = _.reduce(data.requirement, function(obj, req){
          obj.preassigned = _.uniq(_.union(obj.preassigned, req.preassigned), function(pre){
            return pre.courseId.code;
          });

          req.requirements.forEach(function(el, i){
            var type = el.type;
            var index = _.findIndex(obj.requirements, function(r){
              return r.type === type;
            });

            // Have to ensure that its not Base.
            // Everything else overrides base
            if (index > -1) {
              if (req.major !== 'Base') {
                obj.requirements[index] = el;                
              }
            } else {
              obj.requirements.push(el);
            }
          });
        ...
        preassigned = _.reduce(preassigned, function(arr, el){
          ...
          var isExemption = _.indexOf(course.category, 'Exemptions') > -1;
          course.category.splice(0, course.category.length);
          course.category.push(el.category);
          if (isExemption) {
            course.category.push('Exemptions');
          }
          arr.push(course);
          return arr;
        }, []);
        ...
        planner = planner.sort(function(a, b){
          var lengthA = a.courses.length;
          if (a.subtypes.length > 0) {
            var addA = _.reduce(a.subtypes, function(count, el){
              return count + el.courses.length;
            }, 0);

            lengthA += addA;
          }
                   
          var lengthB = b.courses.length;
          if (b.subtypes.length > 0) {
            var addB = _.reduce(b.subtypes, function(count, el){
              return count + el.courses.length;
            }, 0);
